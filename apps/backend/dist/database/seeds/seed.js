...
async function seed() {
    ...
    // Clean old data
    await db_1.db.quizAttempt.deleteMany();
    await db_1.db.question.deleteMany();
    await db_1.db.quiz.deleteMany();
    await db_1.db.lesson.deleteMany();
    await db_1.db.module.deleteMany();
    await db_1.db.classEnrollment.deleteMany();
    await db_1.db.class.deleteMany();
    ...
    // 2. Teacher
    const teacher = await db_1.db.user.create({
        data: {
    ...
    // 3. Student
    const student = await db_1.db.user.create({
        data: {
    ...
    // Create Demo Class
    const demoClass = await db_1.db.class.create({
        data: {
    ...
    await db_1.db.classEnrollment.create({
        data: {
    ...
    // ----------------------------------------------------
    const mod1 = await db_1.db.module.create({
        data: {
    ...
    await db_1.db.lesson.createMany({
        data: [
            {
...
3. **Hydroelectric & Geothermal**: Utilizing water motion and Earth mantle heat.`,
    ...
    const quiz1 = await db_1.db.quiz.create({
        data: {
    ...
    await db_1.db.question.createMany({
        data: [
            {
    ...
    // ----------------------------------------------------
    const mod2 = await db_1.db.module.create({
        data: {
...
5. **Recycle**: Reprocess raw glass, aluminum, and designated plastics.`,
    ...
    const quiz2 = await db_1.db.quiz.create({
        data: {
