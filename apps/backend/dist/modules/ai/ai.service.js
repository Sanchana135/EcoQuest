...
class AiService {
    ...
    static async seedEcoTips() {
        ...
        for (const t of tips) {
            const existing = await db_1.db.ecoTip.findFirst({
                where: { tipText: t.tipText },
            });
            if (!existing) {
                await db_1.db.ecoTip.create({ data: t });
            }
        }
    }
    // 1. AI Assistant Chat & Prompt Engine
    static async processUserPrompt(userId, prompt) {
        ...
        await db_1.db.aiInteractionLog.create({
            data: {
                userId,
                prompt,
    ...
    static async getRecommendations(userId) {
        const attempts = await db_1.db.quizAttempt.findMany({
        ...
        const allModules = await db_1.db.module.findMany({
            include: {
                lessons: true,
                quizzes: true,
            },
    ...
    // 3. Eco Tip Generator
    static async getDailyEcoTip(userId) {
        await this.seedEcoTips();
        const tips = await db_1.db.ecoTip.findMany();
        if (tips.length === 0)
            return null;
        ...
        await db_1.db.userEcoTip.create({
            data: {
                userId,
    ...
    static async getPerformanceAnalysis(userId) {
        const attempts = await db_1.db.quizAttempt.findMany({
            where: { studentId: userId },
            include: {
                quiz: {
