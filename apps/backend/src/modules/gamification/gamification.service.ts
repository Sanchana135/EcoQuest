...
export class GamificationService {
  ...
  static async awardXP(userId: string, amount: number, reason: string, idempotencyKey: string) {
    // Check duplicate transaction
    const existingTx = await db.xPTransaction.findUnique({
    ...
    if (!user) return { awarded: 0, duplicate: false };
  ...
  static async getLeaderboard(period: 'all' | 'weekly' = 'all') {
  ...
  static async getStudentOverview(userId: string) {
    await this.seedBadges();
