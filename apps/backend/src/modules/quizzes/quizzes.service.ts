...
export class QuizzesService {
  ...
  static async submitAttempt(studentId: string, quizId: string, userAnswers: { questionId: string; selectedOption: string }[]) {
    const quiz = await db.quiz.findUnique({
      where: { id: quizId },
    ...
    const newlyUnlockedBadges = await GamificationService.evaluateBadges(studentId);
