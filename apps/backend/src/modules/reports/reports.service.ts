...
export class ReportsService {
  ...
  static async getStudentReport(userId: string) {
    ...
    const levelInfo = GamificationService.calculateLevelInfo(user.xp);
  ...
  static async getTeacherReport(teacherId: string) {
    ...
    for (const s of students) {
      for (const a of s.quizAttempts) {
        ...
        if (!topicStats[cat]) topicStats[cat] = { totalPct: 0, count: 0 };
  ...
  static async getParentReport(studentId?: string) {
    let targetStudentId = studentId;
    if (!targetStudentId) {
      const defaultStudent = await db.user.findFirst({ where: { role: 'STUDENT' } });
  ...
  static async getAdminAnalytics() {
    ...
    const allAttempts = await db.quizAttempt.findMany({ select: { percentage: true } });
